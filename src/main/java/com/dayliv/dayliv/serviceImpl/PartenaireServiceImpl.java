...
public class PartenaireServiceImpl implements PartenaireService {
	...
	public Partenaire save(Partenaire partenaire) {
		...
		Partenaire myPartenaire = partenaireDao.save(partenaire);
		
		if (myPartenaire != null) {
			...
			categories.entrySet().stream().parallel().forEach(e -> {
				...
				catProduct.setStoreCode(myPartenaire.getStoreCode());
				categoryProductDao.save(catProduct);

				e.getValue().forEach(val -> {
					SubCategory subCat = new SubCategory();
					subCat.setSousCategorie(val);
					subCat.setStoreCode(myPartenaire.getStoreCode());
	...
	public Partenaire update(Long id, Partenaire partenaire) {
		...
		if( partenaire.getLongitude() != null) {
			myPartenaire.setLatitude(partenaire.getLatitude());
		}
		
		if(partenaire.getInBusinessSince() !=null) {
			myPartenaire.setInBusinessSince(partenaire.getInBusinessSince());
		}
		if(partenaire.getLongitude() != null) {
			myPartenaire.setLongitude(partenaire.getLongitude());
		}
		
		if(partenaire.getDescription() != null) {
			myPartenaire.setDescription(partenaire.getDescription());
		}
